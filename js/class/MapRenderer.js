var MapRenderer = function (width, height, map, tileset) {
    ...
    this.hoverTileHighlight = new PIXI.DisplayObjectContainer();
...
MapRenderer.prototype._drawGraphics = function () {
    ...
    // hover highlights
    var greenHighlight = new PIXI.Graphics();
    greenHighlight.lineStyle(1, 0x00FF00);
    greenHighlight.drawRect(0, 0, this.tileset.tileWidth - 1, this.tileset.tileHeight - 1);

    var redHighlight = new PIXI.Graphics();
    redHighlight.lineStyle(1, 0xFF0000);
    redHighlight.drawRect(0, 0, this.tileset.tileWidth - 1, this.tileset.tileHeight - 1);

    this.hoverTileHighlight.addChild(greenHighlight);
    this.hoverTileHighlight.addChild(redHighlight);
    this.hoverTileHighlight.green = greenHighlight;
    this.hoverTileHighlight.red = redHighlight;
...
MapRenderer.prototype.click = function (position) {
    ...
    if (!tile || tile == player.tile || distanceBetween(tile.position, player.tile.position) > 1) {
        return;
    }
...
MapRenderer.prototype.hover = function (position) {
    ...
    if (!tile || tile.type == MapTile.TYPE_NULL) {
        show = false;
    }
    ...
    if (show) {
        ...
        if (tile == player.tile || distanceBetween(tile.position, player.tile.position) > 1) {
            this.hoverTileHighlight.green.visible = false;
            this.hoverTileHighlight.red.visible = true;
        } else {
            this.hoverTileHighlight.green.visible = true;
            this.hoverTileHighlight.red.visible = false;
        }

        this.hoverTileHighlight.visible = true;
        Debug.setTilePos(tile.position.x, tile.position.y);
    } else {
        this.hoverTileHighlight.visible = false;
        Debug.setTilePos('-', '-');
    }
...
MapRenderer.prototype.renderLighting = function () {
    ...
    for (y = 0; y < this.map.data.length; y++) {
        ...
        for (x = 0; x < this.map.data[y].length; x++) {
            ...
            } else if (tile.visibility == MapTile.VISIBILITY_PREVIOUS) {
                tile.lightingSprite.alpha = 0.5;
            } else {
...
MapRenderer.prototype.centerOnTile = function (tile) {
